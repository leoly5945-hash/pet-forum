...
class PF_Frontend_Mod {
	...
	public static function enqueue_assets() {
		if ( ! PF_Roles::get_pf_role() && ! current_user_can( 'manage_options' ) ) {
			return;
		}

		wp_enqueue_style(
			'pf-mod-toolbar',
			PF_ROLES_URL . 'assets/css/pf-mod-toolbar.css',
			[],
			PF_ROLES_VER
		);
		wp_enqueue_script(
			'pf-mod-toolbar',
			PF_ROLES_URL . 'assets/js/pf-mod-toolbar.js',
			[ 'jquery' ],
			PF_ROLES_VER,
			true
		);
	...
	public static function render_post_toolbar( $post, $member = null ) {
		...
		if ( ! PF_Roles::user_can_moderate_forum( get_current_user_id(), $forum_id ) ) {
			return;
		}
	...
	public static function ajax_ban_user() {
		...
		$protected = array_merge( [ 'administrator' ], PF_Roles::$pf_role_slugs );
